...
export class Hierarchy {
    ...
    static checkKeys = (item, key) => {
        ...
        if (item.parent) return Hierarchy.checkKeys(item.parent, key);
        else return true;
    };
    ...
    async getHierarchy() {
        ...
        if (
            this.book.hierarchy &&
            ((this.book.hierarchy.version || "-") !== ForgeCompendiumBrowser.version ||
                this.book.version != this.book.hierarchy.bookVersion ||
                this.book.hierarchy.rebuild === true)
        ) {
    ...
    async buildHierarchy() {
        ...
        if (this.book.hierarchy) {
            ...
            await FilePicker.upload(
                src,
                `modules/${this.book.id}/`,
                new File([JSON.stringify(this.book.hierarchy)], "hierarchy.json"),
                {},
                { notify: false }
            );
        }
    ...
    parseFolders(folders, parent) {
        for (const folder of folders) {
            ...
            const _parent = parent || this.getSection(type);
            const child = this.createHierarchyFolder(folder);
    ...
    async parsePacks(packs, parent) {
        ...
        for (const pack of packs) {
            ...
            const _parent = parent || this.getSection(type); // Create the section if the parent doesn't exist
            let folder = _parent;
            ...
            if (olderPackStructure) {
                ...
                const children = this.bookpacks.filter((p) => {
            ...
            if (path) {
                ...
                if (path instanceof Array) {
                    ...
                    for (let i = 0; i < path.length; i++) {
                        ...
                        if (document._id === part.id) break;

                        // see if we can find the folder
                        const parent = folder;
                        const name = decodeURIComponent(part.name);
                        folder = parent.children.find(
                            (c) => c.id === part.id || (part.id == undefined && c.name === name)
                        );
                        if (!folder) {
                        ...
                        if (document.folder && folder.id === document.folder) break;
                    }
                }
            }
        }
    ...
    async parseEntries(pack, folder) {
        ...
        try {
            const type = pack.type || pack.entity;
            const collection = game.packs.get(key);
            ...
            const section = this.hierarchy.children.find((c) => c.packtype === type);
            if (section) {
